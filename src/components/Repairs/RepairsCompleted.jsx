						...
						<div>
							...
							<div className="bg-gray-50 p-4 rounded-lg border border-gray-200 space-y-3 text-sm">
								{[
									...
									<div key={label}>
										{i===0 ? <h3 className="font-semibold text-gray-900">{label}</h3> : (<>
											<span className="text-gray-500">{label}</span>
											<p className="font-medium text-gray-800">{value}</p>
										</>)}
									</div>
						...
						<div>
							...
							<div className="flex flex-wrap gap-4">
								{['Phone Front','Phone Back'].map(img => (
									<img key={img} src={`https://placehold.co/150x150/e0f2fe/3b82f6?text=${encodeURIComponent(img)}`} alt={img} className="w-24 h-24 rounded-lg object-contain border border-gray-200 bg-white p-1" />
								))}
							</div>
						</div>
					</div>
							...
							<div className="bg-gray-50 p-4 rounded-lg border border-gray-200 space-y-3 text-sm">
								...
								<div>
									...
									<div className="flex flex-wrap gap-4 mt-2">
										{['Ref 1','Ref 2'].map(r => (
											<img key={r} src={`https://placehold.co/150x150/e0f2fe/3b82f6?text=${encodeURIComponent(r)}`} alt={r} className="w-24 h-24 rounded-lg object-contain border border-gray-200 bg-white p-1" />
										))}
									</div>
								</div>
							</div>
					...
					<div className="grid grid-cols-1 pt-6">
						<div>
							...
							<div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
								...
								<p className="mt-3 text-sm text-gray-700">Great Job. The service was amazing, the technician was very professional and the shop is very welcoming. I would recommend this shop 100%.</p>
							</div>
						</div>
					</div>
				</div>
				<div className="mt-6 bg-blue-100 p-4 rounded-lg">
					<p className="text-sm text-blue-800 font-semibold">Support Center:</p>
					<p className="text-sm text-blue-800">Having issues with your request? <a href="#" className="font-medium underline hover:text-blue-600">Contact Support</a></p>
				</div>
			</main>
