 ...
 * Updated to filter tickets based on the selected status tab.
 */
...
const AdminSupportTickets = () => {
  ...
  const [activeTab, setActiveTab] = useState('Open'); // Defaulting to 'Open' to match the screenshot
  ...
  const filteredTickets = activeTab === 'All' 
    ? ticketsData 
    : ticketsData.filter(ticket => ticket.status === activeTab);

  // Helper function for Status badge styling
  const getStatusStyle = (status) => {
    switch (status) {
      case 'Open': return 'bg-[#EAEBF0] text-gray-800';
      case 'In Progress': return 'bg-[#EAEBF0] text-gray-800';
  ...
  const getPriorityStyle = (priority) => {
    switch (priority) {
      case 'High': return 'bg-[#EAEBF0] text-gray-800';
